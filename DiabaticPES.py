...
proton_donors = {
    ...
    'H3O+': {'a': 2.145551886772839, 
             'De': dG_H2_diss},
    'Ag': {'a': 1.628, ###only for testing the effect of a values
             'De': 2.27}
    }
...
def fit_morse(df):
    ...
    popt, pcov = curve_fit(morse_norm, df.distance.values, df.energy.values)
...
class PES:
    ...
    def morse_simple(self, r=None):
        ...
        if r is None:
            r = self.df.distance
        if not self.position == 'left':
            return self.De * (self.a * (self.d_Heq - r)-0.5*(self.a * (self.d_Heq - r))**2-1/6*(self.a * (self.d_Heq - r))**3)**2 - self.De
        else:
            return self.De * (self.a * (r - self.d_Heq)-0.5*(self.a * (r - self.d_Heq))**2-1/6*(self.a * (self.d_Heq - r))**3)**2 - self.De
    ...
    def plot_morse_simple(self,
        ...
        ax.plot(x, self.morse(r=x), '--',
                label='Morse: a=%5.3f' % self.a)
        ax.plot(x, self.morse_simple(r=x), '--',
                label='Morse: a=%5.3f' % self.a)
        ax.axhline(color='r', zorder=0, lw=0.6)
        ax.set_xlabel('d ($\mathrm{\AA}$)')
        ax.set_ylabel('E (eV)')
        ax.legend()
        ax.set_title(title)
...
class Energy:
    ...
    def plot_intercepts(self,
        ...
        ax.plot(self.x, self.morse_left(), ls='-.', label='water diabatic PES')
        ax.plot(self.x, self.morse_right(), ls='--', label='metal diabatic PES')
        ax.plot([self.xint], [self.yint], marker='o', markersize=6.0,
                c='grey', markeredgecolor='k', ls='',
                label='E$^{a}_{water}$ = %5.2f eV' % self.Ea_di_left)
        ax.plot([self.xint], [self.yint], marker='o', markersize=6.0,
                c='grey', markeredgecolor='k', ls='',
                label='E$^{a}_{metal}$ = %5.2f eV' % self.Ea_di_right)
        ...
        if adiabatic:
            ...
            ax.plot([self.xint_ad], [self.yint_ad], marker='o', markersize=6.0,
                    ...
                    label='E$^{a}_{water}$ = %5.2f eV' % self.Ea_ad_left)
            ax.plot([self.xint_ad], [self.yint_ad], marker='o', markersize=6.0,
                    c='w', markeredgecolor='b', ls='',
                    label='E$^{a}_{metal}$ = %5.2f eV' % self.Ea_ad_right)
    ...
    def diabatic_intercept(self):
        ...
        if max([self.Ea_di_left, self.Ea_di_right]) < np.abs(self.dG): ##Jiang corrected in the following
            if self.Ea_di_left > self.Ea_di_right:
                self.Ea_di_left = np.abs(self.dG)
                self.Ea_di_right = 0.
            elif self.Ea_di_left < self.Ea_di_right:
                self.Ea_di_left = 0.
                self.Ea_di_right = np.abs(self.dG)
